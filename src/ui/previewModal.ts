...
export class ImportPreviewModal extends Modal {
  ...
  onOpen() {
	...
	selWrap.createEl("label", { text: "Dossier de sortie", attr: { for: "outdir-select" } }).addClass("outdir-label");
	...
	this.outDirSelect.addEventListener("change", () => { this.selectedOutDirRel = this.outDirSelect.value; });
	...
	const updIdentical = (this.summary as any).updated_identical ?? 0;
	const updModified  = (this.summary as any).updated_modified ?? 0;

	const recap = contentEl.createDiv({ cls: "recap-grid" });
	recap.createEl("div", { text: `Nombre d'entrées CSV (hors en-tête) : ${total} (créés + MAJ + erreurs)` });
	...
	if (this.summary.updated > 0) {
	  recap.createEl("div", { text: `• identiques : ${updIdentical}` });
	  recap.createEl("div", { text: `• modifiées : ${updModified}` });
	}
	recap.createEl("div", { text: `Erreurs détectées : ${this.summary.errors}` });

	const btns = contentEl.createDiv({ cls: "modal-button-container" });
	btns.createEl("button", { text: "Annuler" }).addEventListener("click", () => this.close());
	...
	this.registerDomEvent(window, "keydown", (ev: KeyboardEvent) => {
	  ...
	  else if (ev.key === "Enter" && tag !== "select") { ev.preventDefault(); this.confirmBtn?.click(); }
	});
  ...
  private prepareFolderList(existing: string[], defaultRel: string): string[] {
	const set = new Set<string>(existing.map(s => s.replace(/^[\/\\]+|[\/\\]+$/g, "")).filter(Boolean));
  ...
  private makeChip(text: string, variant: "created" | "updated" | "errors", title: string): HTMLElement {
	...
	chip.className = `chip chip-${variant}`; chip.textContent = text; chip.title = title;
	return chip;
  }
  ...
  private injectStyles(): void {
	...
	style.textContent = `
	  ...
	  .import-preview-modal .outdir-select-wrap {
		display: grid; grid-template-columns: 140px 1fr; gap: 8px; align-items: center; margin: 8px 0 8px;
	  }
