...
export async function createRestesFromJournal(app: App): Promise<void> {
	...
	restesInput.wp_import_dataset_key = wpImportDatasetKey || null;
	restesInput.wp_import_dataset_id = wpImportDatasetId;
	...
	const imageName =
		restesInput.img_filename && restesInput.img_filename.length > 0
...
export async function updateRestesFromJournal(app: App): Promise<void> {
	...
	if (!titleChanged && noAChanges) {
		await openInfoModal(
			app,
			"Restes du futur",
			"La note « Restes du futur » est parfaitement à jour !",
		);
		return;
	}
	...
	try {
		...
		if (result.renameTo) {
			...
			if (
				targetName &&
				!eqCaseSensitive(targetName, basenameNoExt(restesFile.name))
			) {
...
function findRestesByPostDate(app: App, postDateIso: string): TFile | null {
	...
	for (const f of files) {
		...
		const isRestes =
			(typeof cat === "string" &&
				cat.trim().toLowerCase() === "restes-du-futur") ||
			(Array.isArray(cat) &&
				cat.some(
					(x) => String(x).trim().toLowerCase() === "restes-du-futur",
				));
